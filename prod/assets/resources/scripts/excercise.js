  ...
  try {
    ...
    if (exercises.length === 0) {
      exerciseList.textContent =
        "Nenhum exercício encontrado para esta parte do corpo.";
      return;
    }
    ...
    exerciseList.textContent =
      "Erro ao carregar os exercícios. Tente novamente mais tarde.";
...
function addExerciseToTraining(exerciseName) {
  ...
  if (training) {
    ...
    showSuccessMessage(
      `Exercício "${exerciseName}" adicionado ao treino "${selectedTraining}"`
    );
  }
}
...
function showSuccessMessage(message) {
  ...
  messageContainer.className =
    "alert alert-success alert-dismissible fade show position-fixed top-50 start-50 translate-middle";
